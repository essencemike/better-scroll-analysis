...
export default class Translater {
  ...
  getComputedPosition() {
    const cssStyle = window.getComputedStyle(this.content, null) as safeCSSStyleDeclaration;

    const matrix = cssStyle[style.transform].split(')')[0].split(', ');
  ...
  translate(point: TranslaterPoint) {
    const transformStyle = [] as string[];
