...
export function createDiv(width: number = 0, height: number = 0, top: number = 0, left: number = 0) {
  const dom = document.createElement('div') as CustomHTMLDivElement;
  mockDomOffset(dom, { width, height, top, left });
  mockDomClient(dom, { width, height });
