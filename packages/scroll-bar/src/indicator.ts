import BScroll, { TranslaterPoint } from '@bsas/core';
import { style, EventEmitter } from '@bsas/shared-utils';
...
export default class Indicator {
  ...
  private hooksHandlers: [EventEmitter, string, Function][] = [];
  ...
  constructor(public bscroll: BScroll, public options: IndicatorOption) {
    ...
    this.listenHooks(options.fade, options.interactive);

    this.refresh();
  }

  private listenHooks(fade: boolean, interactive: boolean) {
    ...
    this.listen(bscrollHooks, 'refresh', this.refresh);
    this.listen(translaterHooks, 'translate', this.updatePosAndSize);
    this.listen(animaterHooks, 'time', (time: number) => {
      this.setTransitionTime(time);
    });
    this.listen(animaterHooks, 'timeFunction', (ease: string) => {
      this.setTransitionTimingFunction(ease);
    });

    if (fade) {
      this.listen(bscrollHooks, 'scrollEnd', () => {
        this.fade();
      });
      this.listen(bscrollHooks, 'scrollStart', () => {
        this.fade(true);
      });

      // 监听鼠标滚轮事件
      if (bscroll.eventTypes.mousewheelStart && bscroll.eventTypes.mousewheelEnd) {
        this.listen(bscrollHooks, 'mousewheelStart', () => {
          this.fade(true);
        });
        this.listen(bscrollHooks, 'mousewheelEnd', () => {
          this.fade();
        });
      }
    }
    ...
    if (interactive) {
      ...
      this.listen(eventHandlerHooks, 'touchStart', this.startHandler);
      this.listen(eventHandlerHooks, 'touchMove', this.moveHandler);
      this.listen(eventHandlerHooks, 'touchEnd', this.endHandler);
    }
  }
  ...
  refresh() {
    const { hasScroll } = this.keysMap;
    if (this.setShowBy(this.bscroll[hasScroll])) {
      const { wrapperSize, scrollerSize, maxScroll } = this.keysMap;

      this.keyVals = this.refreshKeyValues(this.wrapper[wrapperSize], this.bscroll[scrollerSize], this.bscroll[maxScroll]);
  ...
  private setShowBy(hasScroll: boolean): boolean {
    if (hasScroll) {
  ...
  private refreshKeyValues(wrapperSize: number, scrollerSize: number, maxScroll: number): KeyValues {
    const initialSize = Math.max(Math.round((wrapperSize * wrapperSize) / (scrollerSize || wrapperSize || 1)), INDICATOR_MIN_LEN);

    const maxPos = wrapperSize - initialSize;
    const sizeRatio = maxPos / maxScroll;

    return { initialSize, maxPos, sizeRatio };
  }

  updatePosAndSize(endPoint: TranslaterPoint) {
    const { pos, size } = this.refreshPosAndSizeValue(endPoint, this.keyVals);
    this.curPos = pos;
    this.refreshPosAndSizeStyle(size, pos);
  }

  private refreshPosAndSizeValue(endPoint: TranslaterPoint, keyVals: KeyValues): { pos: number; size: number } {
  ...
  private refreshPosAndSizeStyle(size: number, pos: number) {
    const { translate, size: sizeKey } = this.keysMap;

    this.elStyle[sizeKey] = `${size}px`;

    this.elStyle[style.transform as any] = `${translate}(${pos}px)${this.bscroll.options.translateZ}`;
  }
  ...
  moveHandler(moved: boolean, delta: number) {
    ...
    const newScrollPos = this.calScrollDesPos(this.curPos, delta, this.keyVals);
  ...
  private calScrollDesPos(curPos: number, delta: number, keyVals: KeyValues): number {
  ...
  private listen(hooks: EventEmitter, name: string, handler: Function) {
